...
客户端发送, 回应服务端的身份认证信息; 客户端收到401响应后, 要求在请求中包含这个首部
...
为服务器提供客户端想要访问的主机与端口; HTTP/1.1 客户端必须在所有请求中包含 `Host` 首部
...
告知客户端响应已经产生多长时间; HTTP/1.1 缓存必须在发送的每条响应中都包含一个 `Age` 首部
...
通知客户端可以对特定资源使用哪些HTTP方法; 发送 405 Method Not Allowed 响应的 HTTP/1.1 服务器必须包含 `Allow` 首部
...
服务器用来对报文主体进行完整性检查(只有原始服务器或者客户端才可以在报文中插入该首部); 通过这个首部可以端到端的检查数据, 检查在传输过程中是否对数据进行了修改(不可靠)
...
请求传输某范围的文档, 产生的结果由该首部给出, 提供了请求实体所在原始实体的(范围), 并且给出了整个实体的长度; 以 206 Partial Content 响应码进行响应的服务器, 不能包含将"`*`"作为长度使用的 `Content-Range` 首部
...
描述资源的通信选项, 预检; 当前真实请求是非简单请求且跨域的情况下会发起OPTIONS预检请求
...
`永久重定向` 说明请求的资源已经被移动到了由 Location头部指定的url上, 是固定的不会再改变
...
`临时重定向` 请求的资源被暂时的移动到了由该HTTP响应的响应头Location指定的 URL 上

浏览器会重新定向到这个URL,但是搜索引擎不会对该资源的链接更新

推荐仅仅在GET\HEAD 方法采用该状态码
...
通常作为 `PUT` 或 `POST` 操作的返回结果, 它表示重定向链接指向的不是新上传的资源, 而是另外一个页面, 比如消息确认页面或上传进度页面。请求重定向页面的方法要总是使用 `GET`
...
无需再次传输请求的内容, 可以使用缓存的内容, 通常在GET、HEAD中附带If-None-Match 或者If-Modified-Since
