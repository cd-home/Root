...
>   **引入三次握手的主要原因是为了避免过时的重复连接在再次建立时造成的混乱**
>
...
1. 发送方：TCP默认采用的算法是为了避免网络中的报文数量，所以会在缓冲区等待收集多个报文一起发送
2. 接收方：接收方从缓冲读取数据的速率小于缓冲区接收数据的速率，造成多个包被缓存，应用程序读到粘包
